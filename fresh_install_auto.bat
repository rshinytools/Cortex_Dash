@echo off
REM ABOUTME: Automated fresh installation script for Cortex Clinical Dashboard Platform
REM ABOUTME: Non-interactive version for CI/CD and testing

echo ============================================================
echo    CORTEX CLINICAL DASHBOARD - FRESH INSTALLATION (AUTO)
...
echo Step 2: Building fresh Docker images with no cache...
echo ----------------------------------------

REM Build backend with no cache
echo Building backend image (no cache)...
docker build --no-cache -t cortex-backend:prod -f backend/Dockerfile backend/
if %ERRORLEVEL% NEQ 0 (
    echo Backend build failed!
    exit /b 1
)

REM Build frontend with no cache
echo Building frontend image (no cache)...
docker build --no-cache -t cortex-frontend:prod -f frontend/Dockerfile frontend/ --target runner ^
...
docker compose -f docker-compose.local-prod.yml exec -T postgres psql -U postgres -d clinical_dashboard -c "SELECT COUNT(*) as tables FROM information_schema.tables WHERE table_schema = 'public';" 2>nul

REM Check RBAC tables
echo.
echo Checking RBAC tables...
docker compose -f docker-compose.local-prod.yml exec -T postgres psql -U postgres -d clinical_dashboard -c "SELECT COUNT(*) FROM permissions;" 2>nul
docker compose -f docker-compose.local-prod.yml exec -T postgres psql -U postgres -d clinical_dashboard -c "SELECT COUNT(*) FROM roles;" 2>nul

REM Check widgets
echo.
echo Checking widget definitions...
docker compose -f docker-compose.local-prod.yml exec -T postgres psql -U postgres -d clinical_dashboard -c "SELECT code, name FROM widget_definitions;" 2>nul

REM Check organization
echo.
echo Checking organization setup...
docker compose -f docker-compose.local-prod.yml exec -T postgres psql -U postgres -d clinical_dashboard -c "SELECT name FROM organization;" 2>nul

REM Check admin user
echo.
echo Checking admin user setup...
docker compose -f docker-compose.local-prod.yml exec -T postgres psql -U postgres -d clinical_dashboard -c "SELECT email, role, org_id IS NOT NULL as has_org FROM \"user\" WHERE email = 'admin@sagarmatha.ai';" 2>nul
